...
// Input file paths
const inputPath = path.resolve(__dirname, "../../season_data/eyeglasses-raw.json");
const collectionsPath = path.resolve(__dirname, "../../season_data/collections-normalized.json");
// Frame size reference files
const bigsizePath = path.resolve(__dirname, "../../season_data/frame-size-big.json");
const mediumSizePath = path.resolve(__dirname, "../../season_data/frame-size-medium.json");
const smallSizePath = path.resolve(__dirname, "../../season_data/frame-size-small.json");
const metalFramePath = path.resolve(__dirname, "../../season_data/frame-material-metal.json");
const clearanceSalePath = path.resolve(__dirname, "../../season_data/clearance-sale.json");
// Output file path
const outputPath = path.resolve(__dirname, "../../season_data/eyeglasses-normalized.json");
const normalizeNewData = () => {
    ...
    data.products.forEach((product) => {
        ...
        if (bigSizeProduct.some((name) => name.startsWith(baseName))) {
            frameSize = "Big";
        }
        else if (smallSizeProduct.some((name) => name.startsWith(baseName))) {
            frameSize = "Small";
        }
        else if (mediumSizeProduct.some((name) => name.startsWith(baseName))) {
            frameSize = "Medium";
        }
        // If not found in any size list, default to Medium (already set above)
        // Check frame material
        if (metalProduct.some((name) => name.startsWith(baseName))) {
            frameMaterial = "Metal";
        }
        ...
        let price = typeof product.price?.amount === "number"
...
normalizeNewData();
//# sourceMappingURL=normalizeEyeglasses.js.map