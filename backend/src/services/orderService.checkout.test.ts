...
import type { CheckoutInput } from "../types/order.js";
...
test("checkout conflict detector accepts Mongo transaction retry signals", () => {
  ...
  assert.equal(
    isRetryableCheckoutConflict({ codeName: "WriteConflict" }),
    true,
  );
  assert.equal(
    isRetryableCheckoutConflict({
...
test("checkout maps transaction write conflict to a retry-friendly 409", async () => {
  const startSessionDescriptor = Object.getOwnPropertyDescriptor(
    mongoose,
    "startSession",
  );
...
test(
  ...
  async () => {
    ...
    try {
      const product = await Eyeglasses.create({
      ...
      const storedProduct = await Eyeglasses.findById(product._id).lean();
      const storedVariant = storedProduct?.variants.find(
