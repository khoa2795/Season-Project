...
type ProductModel = Model<unknown>;
...
function hasMongoErrorLabel(error: MongoTransactionConflictError, label: string): boolean {
  if (
...
function modelForProduct(productModel: ProductModelName): ProductModel {
  return productModel === "Eyeglasses" ? Eyeglasses : Sunglasses;
}
...
async function findCheckoutProduct(
  ...
  const ProductModel = modelForProduct(item.productModel);
...
function requireCheckoutVariant(
...
async function decrementVariantStock(
  ...
  const ProductModel = modelForProduct(product.productModel);
...
async function restoreOrderItemStock(
  ...
  if (stockUpdate.modifiedCount !== 1) {
    throw new OrderServiceError(
      `Cannot restore stock for order item ${item.variantSku}`,
      409,
    );
  }
}

async function restoreCancelledOrderStock(
  order: IOrder,
  session: ClientSession,
): Promise<void> {
...
export function toCheckoutOrderResponse(order: IOrder): CheckoutOrderResponse {
  return {
...
export async function updateOrderStatusByAdmin(
  ...
  return updateOrderStatus({ _id: orderId }, nextStatus);
}
...
function orderListFilter(
  ...
  return {
    ...(userId === undefined ? {} : { userId: new Types.ObjectId(userId) }),
...
export async function getOrdersForUser(
  ...
  return getOrderList(query, userId);
}
...
export async function cancelOrderForUser(
  ...
  return updateOrderStatus({ _id: orderId, userId }, "cancelled");
}
