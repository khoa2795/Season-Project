  ...
  return (
    <div className="h-full flex flex-col bg-season-menu-bg">
      ...
      <div className="sticky top-0 border-b border-neutral-700 bg-season-menu-bg px-4 sm:px-8 py-6 sm:py-8 z-10 flex items-center justify-between">
        <h2 className="text-lg sm:text-xl font-serif text-white uppercase tracking-mega">
          Season
        </h2>
        {/* Close Button X */}
        <button
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="grid grid-cols-3 gap-4 sm:gap-8 px-4 sm:px-8 py-6 sm:py-8">
          {menuConfig.map((category) => (
            <div
              key={category.name}
              className="space-y-4  text-neutralsm:space-y-6 "
            >
              {/* Category Title - Luxurious Yellow */}
              <h3 className="text-xs sm:text-lg font-medium tracking-wide  text-neutral-300 uppercase">
                {category.name}
              </h3>

              {/* All items together without section titles */}
              <ul className="flex flex-col items-start gap-1 md:gap-2">
                {category.sections.map((section) =>
                  section.items.map((item) => (
                    <li key={item.href} className="w-full">
                      <MenuItem {...item} onItemClick={onClose} />
                    </li>
                  )),
                )}
              </ul>
